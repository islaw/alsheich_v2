...
PAGES_PER_CHUNK = 5
ROUND_NUMBER = 44 # Incremented round number
...
def get_clean_body_text_from_rich_gt(file_path):
    ...
    This version uses the CORRECTED geometric calculation logic.
    """
        ...
        for page in data.get('pages', []):
            ...
            for block in page.get('blocks', []):
                for paragraph in block.get('paragraphs', []):
                    ...
                    # BUG FIX: Use the same logic as the successful diagnostic script.
...
def create_diff_report(gt_text, model_text, file_path):
    ...
    diff = difflib.HtmlDiff(wrapcolumn=80).make_file(
        ...
        fromdesc='אמת מוחלטת (סינון גיאומטרי מתוקן)', todesc='פלט המודל (טקסט נקי)'
    )
...
def main():
    print(f"--- מתחיל הרצה של רתמת בדיקות v2.8, סבב מספר {ROUND_NUMBER} ---")
