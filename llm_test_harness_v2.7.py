...
PAGES_PER_CHUNK = 5
ROUND_NUMBER = 43 # Incremented round number

# Geometric constant: Based on user's measurement of 5.7cm on an A4 page.
# (5.7cm / 29.7cm) * 1122px page height = ~215px. Using 220 for safety.
...
def get_clean_body_text_from_rich_gt(file_path):
    ...
    This version uses the precisely calibrated threshold.
    """
        ...
        for page in data.get('pages', []):
            ...
            for block in page.get('blocks', []):
                for paragraph in block.get('paragraphs', []):
                    ...
                    # We only need the Y coordinate of the top vertices for our threshold check
                    top_y = vertices[0].get('y', 0)
                    
                    pixel_y = top_y * page_height
...
def create_diff_report(gt_text, model_text, file_path):
    ...
    diff = difflib.HtmlDiff(wrapcolumn=80).make_file(
        ...
        fromdesc='אמת מוחלטת (סינון גיאומטרי מכויל)', todesc='פלט המודל (טקסט נקי)'
    )
...
def main():
    print(f"--- מתחיל הרצה של רתמת בדיקות v2.7, סבב מספר {ROUND_NUMBER} ---")
