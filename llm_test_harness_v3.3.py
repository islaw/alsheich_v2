...
# UPDATED: Pointing to the new, stricter prompt
...
PAGES_PER_CHUNK = 5
ROUND_NUMBER = 49 # Incremented round number

# Geometric Constants
HEADER_Y_THRESHOLD = 480 

# Page number box constants (normalized)
PAGE_NUM_X_START = 9.5 / 21.0
PAGE_NUM_X_END = (9.5 + 2) / 21.0
PAGE_NUM_Y_START = 26.8 / 29.7
PAGE_NUM_Y_END = (26.8 + 1) / 29.7
...
def get_structured_and_cleaned_gt(file_path):
    """
    Extracts structured text (with speakers) and page numbers from the
...
def parse_model_output_structured(raw_output):
    """
    Parses the model's multi-page JSON and creates a structured
...
def create_diff_report(gt_lines, model_lines, file_path):
    ...
    diff = difflib.HtmlDiff(wrapcolumn=80).make_file(
        ...
        fromdesc='אמת מוחלטת (מבנה סופי)', todesc='פלט המודל (פרומפט v4.2)'
    )
...
def main():
    print(f"--- מתחיל הרצה של רתמת בדיקות v3.3, סבב מספר {ROUND_NUMBER} ---")
