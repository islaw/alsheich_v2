...
PAGES_PER_CHUNK = 5
ROUND_NUMBER = 42 # Incremented round number

# Geometric constant: Assuming a standard A4 page (29.7 cm height) and 96 DPI resolution.
# 6.3 cm from top is approx 21.2% of the page height.
# Page height in pixels for Google's OCR is often around 1122.
# 1122 * 0.212 = 237.8. We'll use 240 as a safe threshold.
HEADER_Y_THRESHOLD = 240 
...
def get_clean_body_text_from_rich_gt(file_path):
    ...
    using a geometric Y-coordinate threshold to remove headers.
    This version is based on the CORRECT diagnosed structure.
    """
    ...
    try:
        ...
        for page in data.get('pages', []):
            page_height = page.get('dimension', {}).get('height', 1122) # Default A4 height

            for block in page.get('blocks', []):
                for paragraph in block.get('paragraphs', []):
                    # Check the Y-coordinate of the paragraph's bounding box
                    avg_y = 0
                    ...
                    if pixel_y > HEADER_Y_THRESHOLD:
                        # This paragraph is part of the body, extract its text
                        paragraph_text = ""
                        for segment in paragraph.get('layout', {}).get('textAnchor', {}).get('textSegments', []):
...
def create_diff_report(gt_text, model_text, file_path):
    ...
    diff = difflib.HtmlDiff(wrapcolumn=80).make_file(
        ...
        fromdesc='אמת מוחלטת (סינון גיאומטרי)', todesc='פלט המודל (טקסט נקי)'
    )
...
def main():
    print(f"--- מתחיל הרצה של רתמת בדיקות v2.6, סבב מספר {ROUND_NUMBER} ---")
    
    # 1. Load and Clean Ground Truth from RICH JSON
